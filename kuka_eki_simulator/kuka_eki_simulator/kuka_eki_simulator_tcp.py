    ...
    try:
        ...
        while rclpy.ok():
            ...
            try:
                ...
                str_data = create_eki_xml_rob(act_joint_pos, act_command_id, disig=disig, dosig=dosig)
                msg = String()
                ...
                str_data = create_eki_xml_rob(act_joint_pos, act_command_id, True, disig=disig, dosig=dosig)
                msg = String()
                msg.data = str(str_data)
                eki_act_pub.publish(msg)
                conn_motion.send(str_data)  # Send data over TCP
